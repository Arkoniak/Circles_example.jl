...
using ImageMorphology.FeatureTransform: nullindex, DistRFT, computeft!
using Images
using Luxor
using Statistics
...
function calc_label!(circles, mask, min_radius, left, top, img)
    ...
    R = CartesianIndices(axes(mask))
    w, h = size(mask)
    ∅ = nullindex(R)::CartesianIndex{2}
    F = similar(mask, CartesianIndex{2})
    tmp = Vector{DistRFT{2, Int64}}()
    while true
        empty!(tmp)
        # Compute the feature transform (recursive algorithm)
        computeft!(F, mask, CartesianIndex(()), nothing, tmp)
        ...
        clr = circle_indices(mask, offset, img, ix, r, w, h)
